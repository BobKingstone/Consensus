...
class ConsensusEngine:
    ...
    def load_evaluators(self):
        ...
        evaluators = ConfigHelper.get_config_section_values(
            "Servers", "evaluators"
        ).split()
        i = 0
        for e in evaluators:
        ...
        while i < count:
            ...
            for ev in self.__evaluators:
                with grpc.insecure_channel(ev.address) as channel:
                    ...
                    evaluator_predictions.append(
                        ParticipantResponse(evaluator_prediction, evaluator_experience)
                    )
            i += 1

            prediction = self.ensembler.get_ensemble_result(
                participant_responses=evaluator_predictions
            )
            self.update_accuracy(prediction, self.__sample_repo.get_last_sample_type())
            print(
                prediction,
                " last sample type ",
                self.__sample_repo.get_last_sample_type(),
            )
        ...
        while i < round_count:
            ...
            for ev in self.__evaluators:
                with grpc.insecure_channel(ev.address) as channel:
                    ...
                    if (
                        response.decision
                        is evaluator_pb2.EvaluationDecision.Decision.BENIGN
                    ):
                        # print("Sample marked as BENIGN")
                        benign += 1
                    elif (
                        response.decision
                        is evaluator_pb2.EvaluationDecision.Decision.ANOMALY
                    ):
