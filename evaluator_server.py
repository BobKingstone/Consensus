...
class EvaluationAgent:
    ...
    def load_model(self):
        ...
        model_path = ConfigHelper.get_config_section_values(
...
class Evaluator(evaluator_pb2_grpc.EvaluatorServicer):
    ...
    def ndarray_to_proto(
        dec: EvaluationDecision.Decision, nda: np.ndarray, experience: int
    ) -> EvaluationDecision:
        ...
        return EvaluationDecision(
            decision=dec, ndarray=nda_bytes.getvalue(), experience=experience
        )
...
def serve():
    ...
    port = ConfigHelper.get_config_section_values(
        "ServerPorts", "port_" + str(server_id)
    )
